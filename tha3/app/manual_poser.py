...
# 添加当前工作目录到 sys.path
# os.getcwd() 用于获取执行 python 脚本时所在的工作目录，返回的是一个代表当前工作目录的字符串
...
class MorphCategoryControlPanel(wx.Panel):
    # python 的构造函数，用于初始化 MorphCategoryControlPanel 类的实例。并显示 UI
    # self 是指向当前实例
    def __init__(self,
                 parent,
                 title: str,
        ...
        # 调用父类的构造函数，使用 super()，super 是
        super().__init__(parent, style=wx.SIMPLE_BORDER)
        # 进行成员变量的初始化
        # pose_param_category 是 PoseParameterCategory 类型的参数，表示这个控制面板对应的姿势参数类别
        # 这个是项目自定义的类型，在 tha3/poser/poser.py 中定义
        self.pose_param_category = pose_param_category
        # sizer 用于控制 wxPython 中的布局，结合最终的运行效果，这里使用的是一个垂直布局方式
        self.sizer = wx.BoxSizer(wx.VERTICAL)
        # 将 sizer 设置为当前面部的布局管理器
...
class MainFrame(wx.Frame):
    ...
    def draw_nothing_yet_string_to_bitmap(self, bitmap):
        ...
        text_to_draw = getattr(self, "source_image_string", "Nothing yet!")
        w, h = dc.GetTextExtent(text_to_draw)
        dc.DrawText(text_to_draw, (self.image_size - w) // 2, (self.image_size - h) // 2)
